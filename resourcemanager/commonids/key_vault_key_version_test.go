...
func TestFormatKeyVaultKeyVersionID(t *testing.T) {
	actual := NewKeyVaultKeyVersionID("12345678-1234-9876-4563-123456789012", "example-resource-group", "vaultValue", "keyValue", "versionValue").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.KeyVault/vaults/vaultValue/keys/keyValue/versions/versionValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseKeyVaultKeyVersionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *KeyVaultKeyVersionId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		{
			// Incomplete URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.KeyVault/vaults/vaultValue/keys/keyValue/versions",
			Error: true,
		},
		{
			// Valid URI
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.KeyVault/vaults/vaultValue/keys/keyValue/versions/versionValue",
			Expected: &KeyVaultKeyVersionId{
		...
		},
		{
			...
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.KeyVault/vaults/vaultValue/keys/keyValue/versions/versionValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseKeyVaultKeyVersionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseKeyVaultKeyVersionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *KeyVaultKeyVersionId
	...
	for _, v := range testData {
		...
		actual, err := ParseKeyVaultKeyVersionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForKeyVaultKeyVersionId(t *testing.T) {
	segments := KeyVaultKeyVersionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("KeyVaultKeyVersionId has no segments")
	}
