...
func TestNewKeyVaultKeyID(t *testing.T) {
	id := NewKeyVaultKeyID("12345678-1234-9876-4563-123456789012", "example-resource-group", "vaultValue", "keyValue")
...
func TestFormatKeyVaultKeyID(t *testing.T) {
	actual := NewKeyVaultKeyID("12345678-1234-9876-4563-123456789012", "example-resource-group", "vaultValue", "keyValue").ID()
...
func TestParseKeyVaultKeyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *KeyVaultKeyId
	}{
		{
			// Incomplete URI
			Input: "",
	...
	for _, v := range testData {
		...
		actual, err := ParseKeyVaultKeyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseKeyVaultKeyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *KeyVaultKeyId
	...
	for _, v := range testData {
		...
		actual, err := ParseKeyVaultKeyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForKeyVaultKeyId(t *testing.T) {
	segments := KeyVaultKeyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("KeyVaultKeyId has no segments")
	}
