...
func TestFormatSpringCloudServiceID(t *testing.T) {
	actual := NewSpringCloudServiceID("12345678-1234-9876-4563-123456789012", "example-resource-group", "serviceValue").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.AppPlatform/spring/serviceValue"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseSpringID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		...
		},
		{
			...
			Expected: &SpringCloudServiceId{
				...
				ServiceName:       "springValue",
			},
		},
		{
			// Invalid (Valid Uri with Extra segment)
			Input: "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/example-resource-group/providers/Microsoft.AppPlatform/spring/springValue/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseSpringCloudServiceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseSpringIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SpringCloudServiceId
	...
	for _, v := range testData {
		...
		actual, err := ParseSpringCloudServiceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForSpringId(t *testing.T) {
	segments := SpringCloudServiceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("SpringCloudServiceId has no segments")
	}
